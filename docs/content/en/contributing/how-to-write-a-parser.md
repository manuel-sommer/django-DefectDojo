...
In order to write a parser you need to handle various file formats. This can be achieved with the use of libraries. However, in order to keep DefectDojo slim and also don't extend the attack surface, keep the number of libraries used to parse information slim and take other parsers as an example.
